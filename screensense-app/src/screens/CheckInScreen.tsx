...
export default function CheckInScreen({ userId = 'user_001', userName = 'Harrison', onComplete }: Props) {
  ...
  const submit = async () => {
    ...
    try {
      const res = await api.checkin({
        user_id: userId, mood_label: mood.toLowerCase(),
        mood_words: thoughts.filter(t => t !== 'None right now'),
        screen_time_hours: 4.0, scroll_session_mins: 15,
        sleep_hours: 7.0, energy_level: energy,
  ...
  if (result) return (
    <ResultScreen
      result={result} mood={mood!}
      onReset={() => { setStep(0); setMood(null); setResult(null); setJournal(''); setBody([]); setThoughts([]); }}
    />
  );

  const sel = MOODS.find(m => m.label === mood);
  const progressWidth = progress.interpolate({ inputRange: [0, 1], outputRange: ['0%', '100%'] });
  const canNext = [true, !!mood, true, true, true, true][step] ?? true;
  ...
  return (
    <View style={s.root}>
      ...
      <View style={s.navRow}>
        {step > 0 ? (
          <TouchableOpacity style={s.navBtn} onPress={() => transition(step - 1)} hitSlop={{ top: 12, bottom: 12, left: 12, right: 12 }}>
            <Text style={s.navBtnTxt}>←</Text>
          </TouchableOpacity>
        ) : <View style={s.navBtn} />}
        <Text style={s.stepLbl}>Step {step + 1} of {TOTAL}</Text>
        <TouchableOpacity style={s.crisisBtn} onPress={() => {}}>
      ...
      <Animated.View style={[s.stepArea, { opacity, transform: [{ translateY }] }]}>
        ...
        {step === 0 && (
          <View style={s.step}>
            ...
            <Text style={[Font.body, s.stepBody]}>A 2-minute check-in that adapts to your mood, context, and history.</Text>
        ...
        {step === 2 && (
          <View style={s.step}>
            ...
            <View style={s.bigNumberRow}>
              <Text style={[Font.display, { color: sel?.color || C.violet, fontSize: 80, lineHeight: 84 }]}>{intensity}</Text>
            ...
            <View style={s.step}>
              ...
              {thoughts.length > 0 && !thoughts.includes('None right now') && (
                <View style={s.insightPill}>
                  <Text style={s.insightTxt}>The Thought Challenger in Therapy can help reframe these patterns.</Text>
                </View>
              )}
            </View>
          </ScrollView>
        ...
        {step === 5 && (
          <View style={s.step}>
            <Text style={[Font.label, s.stepLabel]}>VADER sentiment analysis</Text>
            <Text style={[Font.h1, s.stepTitle]}>Anything on{'\n'}your mind?</Text>
            <Text style={[Font.body, s.stepBody]}>Optional. Your words will be processed by sentiment analysis for deeper insight.</Text>
            <TextInput
              style={s.journal}
              placeholder={`What's been happening today, ${userName.split(' ')[0]}?`}
              placeholderTextColor={C.textGhost}
              multiline value={journal}
              onChangeText={setJournal}
              maxLength={500}
            />
            <Text style={[Font.micro, { textAlign: 'right', marginTop: Space['2'] }]}>{journal.length} / 500</Text>
...
const s = StyleSheet.create({
  ...
  featureRow: { flexDirection: 'row', gap: Space['2'], flexWrap: 'wrap', marginTop: Space['8'] },
